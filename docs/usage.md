...
本文说明当前代码中已经实现的 API 用法。示例以 crate 用户视角编写，类型名、模块路径和行为均按当前源码确认；设计文档中提到但源码未公开或未实现的接口不会写入本文。
...
部分配置类型没有在 crate 根 re-export，需要从子模块导入：
...
所有 key 和 value 都是 bytes。`put`、`delete`、`get`、`scan` 等公开 API 接收 `&[u8]`，返回值使用 `Vec<u8>`。
...
`Options::default()` 会在目录不存在时创建数据库。打开已有目录时，如果目录下有 `CURRENT`，会通过 `MANIFEST-*` 和当前 WAL 恢复状态；否则会创建新的 `VersionSet`、`CURRENT` 和 MANIFEST。
...
- `error_if_exists = true`：如果目标路径已经存在，`DB::open` 返回 `Error::InvalidArgument`。
- `create_if_missing = false`：如果目标路径不存在，`DB::open` 返回 `Error::InvalidArgument`。

`DB` 是可 clone 的轻量句柄，内部状态通过 `Arc` 共享。

## 基础读写

`put` 写入或覆盖 key，`delete` 写入 tombstone，`get` 返回最新可见值：
...
空 key、二进制 key、二进制 value 都按 bytes 处理。调用方不需要传 UTF-8 字符串。

## 批量写入

`WriteBatch` 是一次原子写入单位。DB 会给 batch 内记录分配连续 sequence number，并按 batch 顺序应用：
...
- `sync = true`：本次写入要求 WAL append 后执行 sync。
- `disable_wal = true`：在全局 WAL 启用时跳过本次写入的 WAL。除非之后 flush 并通过 MANIFEST 持久化，否则 crash 后可能丢失这次写入。

如果 batch 为空，`DB::write` 直接返回 `Ok(())`。
...
- `wal_enabled = true`：写入会先追加到 WAL，再进入 memtable。
- `wal_sync = WalSyncMode::Never`：默认值，写入不会每次自动 fsync。
- `wal_sync = WalSyncMode::PerWrite`：每次写入都会触发 WAL sync，并且 group commit 会等待 `write_group_max_delay` 聚合并发写。
...
`get_opt` 和 `scan_opt` 接收 `ReadOptions`。最常用的是带 `Snapshot` 的一致性读：
...
fn snapshot_read() -> Result<()> {
    ...
    assert_eq!(db.get_opt(b"k", read_options)?, Some(b"old".to_vec()));
...
`Snapshot` 保存创建时的 `read_seq`，clone 会登记同一个 read sequence，drop 会释放 watermark 引用。活跃 snapshot 会影响 compaction GC：低于活跃 snapshot 仍可能可见的旧版本不会被删除。

`ReadOptions` 字段：

- `snapshot: Option<Snapshot>`：指定读序列；不传时读取 DB 当前最新 sequence。
- `fill_cache: bool`：控制 SSTable block 读取时是否使用/填充 block cache。
- `verify_checksums: bool`：当前读路径没有分支使用该字段；SSTable block 解码时会固定校验 checksum。
- `total_order_seek: bool`：当前实现没有使用该字段。
...
- 如果事务读过的 key 在事务 snapshot 之后被其他写入修改，`commit` 返回 `Error::TransactionConflict`。
- 如果事务准备写入的 key 在事务 snapshot 之后被其他写入修改，`commit` 返回 `Error::TransactionConflict`。
- 如果事务 scan 过的 range 在事务 snapshot 之后发生插入/删除/更新，`commit` 返回 `Error::TransactionConflict`。
- 事务 commit 会把事务内 `WriteBatch` 原子写入 DB。
- `Transaction::commit(self)` 和 `Transaction::rollback(self)` 会消费事务对象。
...
Compaction 会根据活跃 snapshot watermark 决定哪些旧版本可以删除。不要在仍需要旧版本读取的 snapshot 存活时期待 compaction 删除这些旧版本。
...
- `block_size`：SSTable builder 的 data block 目标大小。
- `target_file_size_base`：compaction 输出拆分的目标大小。
...
`metrics_snapshot` 返回累计指标，并包含 block cache stats：
...
- `Error::Corruption(String)`：WAL、MANIFEST、SSTable、锁 poisoning 等损坏/不一致情况。
- `Error::Io(std::io::Error)`：文件系统错误。
- `Error::Closed`：DB 或 transaction 已关闭。
- `Error::Unsupported(&'static str)`：当前源码中定义了该 variant，主路径很少使用。
